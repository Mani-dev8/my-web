...
const getResolvedPosition = (position?: FadedCircleProps['position']): CSSProperties => {
    ...
    return {
        top: hasCustomTop ? position?.top : (!hasCustomBottom ? DEFAULT_POSITION.top : undefined),
        bottom: hasCustomBottom ? position?.bottom : undefined,
        left: hasCustomLeft ? position?.left : (!hasCustomRight ? DEFAULT_POSITION.left : undefined),
...
function FadedCircle({ size, position, color, className }: FadedCircleProps) {
    ...
    return (
        <motion.div
            className={`absolute rounded-full opacity-40 blur-3xl pointer-events-none${className ? ` ${className}` : ''}`}
            style={circleStyle}
            animate={{
                x: offset.x,
                y: offset.y
